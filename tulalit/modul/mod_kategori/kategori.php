...
switch($_GET[act]){
  // Tampil Kategori
  default:
    echo "<h2>Kategori Jurusan</h2>
          <input type=button class='tombol' value='Tambah Kategori Jurusan' 
          onclick=\"window.location.href='?module=kategori&act=tambahkategori';\">
          <table>
          <tr><th>No</th><th>Nama Kategori Jurusan</th><th>Aksi</th></tr>"; 
    $tampil=mysql_query("SELECT * FROM kategori WHERE id_kategori != '666' AND id_kategori != '999' ORDER BY id_kategori DESC");
    $no=1;
  ...
  case "tambahkategori":
    echo "<h2>Tambah Kategori Jurusan</h2>
          <form method=POST action='$aksi?module=kategori&act=input'>
          <table>
          <tr><td>Nama Kategori Jurusan</td><td> : <input type=text name='nama_kategori'></td></tr>
  ...
  case "editkategori":
    ...
    echo "<h2>Edit Kategori Jurusan</h2>
          <form method=POST action=$aksi?module=kategori&act=update>
          <input type=hidden name=id value='$r[id_kategori]'>
          <table>
          <tr><td>Nama Kategori Jurusan</td><td> : <input type=text name='nama_kategori' value='$r[nama_kategori]'></td></tr>
