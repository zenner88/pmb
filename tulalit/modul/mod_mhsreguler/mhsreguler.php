...
$aksi="modul/mod_mhsreguler/aksi_mhsreguler.php";
...
<form method=get action='$_SERVER[PHP_SELF]'>
          <input type=hidden name=module value=mhsreguler>
...
<hr>
		  ...
		  <form method='POST' action='modul/mod_cetak/lokasiujian.php' target='_blank'>
          <input type=hidden name=module value=daftar>
          <div id=paging>
		  <B>LOKASI UJIAN :</B>";

  $query_lks=mysql_query("select * from t_tempat_ujian");

				echo"  <select name='lokasi' size='1' >
				   <option selected='selected' value='Semua'>Cetak Semua</option>";
                            while ($row_lks=mysql_fetch_array($query_lks))
   							{
                            echo"<option value ='$row_lks[KodeTmp]'>$row_lks[NamaTmp]</option>";
                             }
						echo"
                        </select>	
...
<form method='POST' action='modul/mod_cetak/calonmahasiswareguler.php' target='_blank'>
          <input type=hidden name=module value=daftar>
          <div id=paging>
...
<form method='POST' action='../tulalit/excel/pendaftarreguler.php' target='_blank'>
          <input type=hidden name=module value=daftar>
          <div id=paging>
		  <B>PERGURUAN TINGGI : </B> <input name='pt' value='all' checked type='radio' >Cetak Semua 
<input name='pt' value='poltekpos' type='radio' >Poltekpos
...
<form method=POST action='?module=mhsreguler&act=lulus'>
         <B>UPLOAD KELULUSAN REGULER: </B> <input type=submit class='tombol' value='Upload'>
</form>
...
echo "<table>
		...
		<tr>
		<th>No</th><th>No.Registrasi</th><th>Perguruan Tinggi</th><th>Nama Lengkap</th><th>Tempat Ujian</th><th>Status</th><th>Pindah Perguruan Tinggi</th></tr>";

	$p = new Paging;
	$batas = 10;
	$posisi = $p->cariPosisi($batas);
	$tampil = mysql_query("SELECT t_calon_mahasiswa.*, t_tempat_ujian.*,t_daftar.*
...
?>
	...
	<td><?=$no?></td>
	<td><a href='?module=mhsreguler&act=lihat_reguler&id=<?=$r['i_registrasi']?>'><?=$r['i_registrasi']?></a></td>
	<?php
	if ($r['pilihan'] == "1")
	{
...
<?php
	...
	echo "<td><a href='?module=mhsreguler&act=pindah&id=".$r['i_registrasi']."'>Pindah</a></td></tr>";
	$no++;
}
	echo "</table>";
...
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi) 
and jalur_pendaftaran='1' and pilihan='1' and status='bayar'");
...
WHERE t_calon_mahasiswa.c_gel<>'PMDK' and t_jurusan.Jenjang='1'
group by t_jurusan.NamaJrsn
order by t_jurusan.NamaJrsn");
$no = $posisi + 1;

echo"
<b>Statistika Poltekpos</b>
<table> 
...
WHERE kode_briva not in (select t_calon_mahasiswa.i_registrasi from t_calon_mahasiswa
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi) 
and jalur_pendaftaran='1' and pilihan='2' and status='bayar'");
...
FROM t_calon_mahasiswa
inner join t_jurusan on t_jurusan.KodeJrsn=t_calon_mahasiswa.n_pil1
WHERE t_calon_mahasiswa.c_gel<>'PMDK' and t_jurusan.Jenjang='2'
group by t_jurusan.NamaJrsn
order by t_jurusan.NamaJrsn");
$no = $posisi + 1;

echo"
<b>Statistika Stimlog</b>
<table> 
<tr><th>No</th><th>Program Studi</th>
<th>Jumlah Daftar</th>
</tr>";
...
break;    
	...
	$tampil = mysql_query("SELECT t_calon_mahasiswa.*, t_tempat_ujian.*, t_daftar.*
						...
						INNER JOIN t_tempat_ujian ON t_calon_mahasiswa.i_temp_ujian = t_tempat_ujian.KodeTmp
						inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi
						WHERE t_calon_mahasiswa.c_jalur = 'reguler' and t_calon_mahasiswa.n_lengkap like '%$_GET[kata]%'
						LIMIT $posisi,$batas");
	$no = $posisi + 1;
	while ($r = mysql_fetch_array($tampil))
	{
...
<?php
	...
	echo "</table>";
	$jmldata = mysql_num_rows(mysql_query("SELECT * FROM t_calon_mahasiswa WHERE c_jalur='reguler' and n_lengkap like '%$_GET[kata]%'"));
	$jmlhalaman = $p->jumlahHalaman($jmldata, $batas);
	$linkHalaman = $p->navHalaman($_GET[halaman], $jmlhalaman);
	echo "<div id=paging>$linkHalaman</div><br>";
...
<td><b>$r_belum_s[jum]</b></td>
  ...
  case "lihat_reguler":
   $edit = mysql_query("SELECT t_calon_mahasiswa.*,t_informasi.NamaInf,t_kerja_ortu.n_kerja,t_tempat_ujian.*,t_prop.*,t_kab.* FROM t_calon_mahasiswa INNER JOIN t_tempat_ujian ON t_calon_mahasiswa.i_temp_ujian=t_tempat_ujian.KodeTmp INNER JOIN t_prop ON t_prop.kd_prop = t_calon_mahasiswa.n_propinsi INNER JOIN t_kab ON t_kab.kd_kab = t_calon_mahasiswa.n_kabupaten
   inner join t_informasi on t_informasi.KodeInf=t_calon_mahasiswa.c_inf 
 			...
 			<tr>
    			<td>
					<table>
						...
						<tr><td>Tahu Informasi dari</td><td> : <?=$r[NamaInf]?></td></tr>
  			...
  			<tr>
    			<tr><td colspan=2 align=center><input type="button" value="Kembali" onclick='self.history.back()' style="cursor:pointer">
						<a href="?module=mhsreguler&act=update&id=<?=$r[i_registrasi]?>" style="text-decoration:none"><input type="button" value="Edit" style="cursor:pointer"></a></td></tr>
...
?>
	...
	<? echo "
		  <form method=POST enctype='multipart/form-data' action='$aksi?module=mhsreguler&act=editreg' name='demo'>";?>
		<table>
 			<tr>
    			<td>
					<table>
...
echo"
    <h2>Input Kelulusan Reguler</h2>
	<form method=POST enctype='multipart/form-data' action='$aksi?module=mhsreguler&act=inputmhsexcel'>
...
echo"
	...
	<form method='POST' enctype='multipart/form-data' action='$aksi?module=mhsreguler&act=pindah'>
