...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Berita</h2>
          ...
          <input type=button class='tombol' value='Tambah Berita' onclick=\"window.location.href='?module=berita&act=tambahberita';\">";
    ...
    if ($_SESSION['leveluser']=='admin'){
      ...
      $tampil=mysql_query("SELECT * FROM berita 
                           WHERE id_kategori='$_SESSION[leveluser]'       
    ...
    if ($_SESSION['leveluser']=='admin'){
      ...
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM berita WHERE id_kategori='$_SESSION[leveluser]'"));
    }  
    ...
    if ($_SESSION['leveluser']=='admin'){
      ...
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM berita WHERE id_kategori='$_SESSION[leveluser]' AND judul LIKE '%$_GET[kata]%'"));
    }  
    $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
    $linkHalaman = $p->navHalaman($_GET[halaman], $jmlhalaman);
  ...
  case "tambahberita":
	...
	echo "<tr><td>Headline</td>    <td> : <input type=radio name='headline' value='Y' checked>Y  
          ...
          <tr><td>Isi Berita</td>  <td> <textarea id='ckeditor' class='ckeditor' name='isi_berita'  style='width: 600px; height: 350px;'></textarea></td></tr>
  ...
  case "editberita":
	...
	if ($_SESSION['leveluser']=='admin'){
    echo "<tr><td>Kategori</td>  <td> : <select name='kategori'>";
   ...
   if ($r[headline]=='Y'){
      ...
      echo "<tr><td>Isi Berita</td>   <td> <textarea id='ckeditor' class='ckeditor' name='isi_berita' style='width: 600px; height: 350px;'>$r[isi_berita]</textarea></td></tr>
          <tr><td>Gambar</td>       <td> :  ";
          if ($r[gambar]!=''){
