...
for ( $i = ($lastId+1) ; $i <= $x ; $i++) {
    ...
    mysql_query("INSERT INTO t_pin(pin,kd_aktivasi,status)
                            VALUES('$inc','$kdAktivasi','nonaktif')");
                            }
...
$row_tampil=mysql_fetch_array($sqlTahun);
  $thn = substr($row_tampil[1],2,3);
  $reg = $thn."1";
    $sqlLast = mysql_query("SELECT * FROM t_pin WHERE pin REGEXP CONVERT( _utf8 '$reg' USING latin1 ) COLLATE latin1_swedish_ci ORDER BY pin DESC LIMIT 1;");
$row_last=mysql_fetch_array($sqlLast);
if(mysql_num_rows($sqlLast)>0){
	//$lastId = substr($row_last[1],3,4);v
	$lastId  = $row_last[1];
