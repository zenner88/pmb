...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Tahun Akademik</h2>
          <input type=button value='Tambah Tahun Akademik' onclick=\"window.location.href='?module=ta&act=tambahta';\">
          <table>
   ...
   // }
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr class=menucell onMouseDown=pviiClassNew(this,'mousedown') onMouseOut=pviiClassNew(this,'menucell') onMouseOver=pviiClassNew(this,'mouseover')>
				...
				<td>$r[n_akademik]</td>";
		if($r[status]=="on") {
				echo"<td align=center ><img src='icon/aktif.png' height=20 weight=40></td>";
		} else {
				echo"<td align=center><img src='icon/nonaktif.jpg' height=20 weight=40></td>";
		}
		echo"<td><a href=?module=ta&act=editta&id=$r[id_thn_akademik]>Edit</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahta":
    echo "<h2>Tambah Tahun Akademik</h2>
          <form method=POST action='$aksi?module=ta&act=input' enctype='multipart/form-data'>
          <table>
  ...
  case "editta":
    ...
    echo "<h2>Ubah Jenjang Program Studi</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?module=ta&act=update>
