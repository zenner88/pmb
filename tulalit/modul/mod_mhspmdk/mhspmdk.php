...
$aksi="modul/mod_mhspmdk/aksi_mhspmdk.php";
switch($_GET[act]){
...
echo"
	<h2>Data Pendaftaran PMDK</h2>
	<table>
	
 <form method=get action='$_SERVER[PHP_SELF]'>
          <input type=hidden name=module value=mhspmdk>
...
<form method='POST' action='modul/mod_cetak/calonmahasiswapmdk.php' target='_blank'>
...
<input name='pt' value='stimlog' type='radio' >Stimlog
		  ...
		  <input type=submit class='tombol' value=Tampil></div>
          </form>

<form method='POST' action='../tulalit/excel/pendaftarpmdk.php' target='_blank'>
          <input type=hidden name=module value=daftar>
          <div id=paging>
		  <B>PERGURUAN TINGGI : </B> <input name='pt' value='all' checked type='radio' >Cetak Semua 
<input name='pt' value='poltekpos' type='radio' >Poltekpos
...
<form method=POST action='?module=mhspmdk&act=lulus'>
         <B>UPLOAD KELULUSAN PMDK : </B> <input type=submit class='tombol' value='Upload'>
</form>
...
WHERE t_calon_mahasiswa.c_jalur='pmdk' AND t_daftar.status='bayar' LIMIT $posisi,$batas");
$no = $posisi + 1;
...
?>
	...
	<td><a href='?module=mhspmdk&act=lihat_pmdk&id=<?=$r[i_registrasi]?>'><?=$r[i_registrasi]?></a></td>
	<?php
	if ($r['pilihan'] == "1")
	{
	...
	echo"<td><a href='?module=mhspmdk&act=pindah&id=$r[i_registrasi]'>Pindah</a></td></tr>";
	$no++;
}
echo "</table>";
...
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi) 
and jalur_pendaftaran='0' and pilihan='1' and status='bayar'");
...
WHERE t_calon_mahasiswa.c_jalur='PMDK' and t_jurusan.Jenjang='1'
...
WHERE kode_briva not in (select t_calon_mahasiswa.i_registrasi from t_calon_mahasiswa
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi) 
and jalur_pendaftaran='0' and pilihan='2' and status='bayar'");
...
FROM t_calon_mahasiswa
inner join t_jurusan on t_jurusan.KodeJrsn=t_calon_mahasiswa.n_pil1
WHERE t_calon_mahasiswa.c_jalur='PMDK' and t_jurusan.Jenjang='2'
group by t_jurusan.NamaJrsn
order by t_jurusan.NamaJrsn");
$no = $posisi + 1;
...
$tampil = mysql_query("SELECT t_calon_mahasiswa.*, t_daftar.* FROM t_calon_mahasiswa 
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi
WHERE t_calon_mahasiswa.c_jalur='pmdk' and t_calon_mahasiswa.n_lengkap like '%$_GET[kata]%'LIMIT $posisi,$batas");
$no = $posisi + 1;
while ($r = mysql_fetch_array($tampil))
{
?>
...
<td><b>$r_belum_s[jum]</b></td>
  ...
  $r    = mysql_fetch_array($edit);
	...
	?>
	  <h2>Detail Data Pendaftaran PMDK</h2>
  			...
  			<tr>
    			...
    			<td>
					<table><h3>Data SMA Calon Mahasiswa</h3>
						...
						<tr><td>Nilai Rata kls XI smstr II</td> <td> : <?=$r[rata2_XI_2]?></td></tr>
						<tr><td>Nilai Matematika kls XI</td> 	<td> : <?=$r[mtk_XI_2]?></td></tr>
						<tr><td>Nilai Bhs.Inggris kls XI</td> 	<td> : <?=$r[ing_XI_2]?></td></tr>
						<tr><td>Nilai Rata kls XII smstr I</td> <td> : <?=$r[rata2_XII_1]?></td></tr>
						<tr><td>Nilai Matematika kls XII</td> 	<td> : <?=$r[mtk_XII_1]?></td></tr>
						<tr><td>Nilai Bhs.Inggris kls XII</td> 	<td> : <?=$r[ing_XII_1]?></td></tr>
<tr><td>Scan Nilai Raport Semester IV</td> 	<td> :<a href="../peserta/smt11/<?=$r[smt11];?>" onClick="return GB_showCenter('Tentang Web', this.href)"><?=$r[smt11];?></a></td></tr>
<tr><td>Scan Nilai Raport Semester IV</td> 	<td> :<a href="../peserta/smt12/<?=$r[smt11];?>" onClick="return GB_showCenter('Tentang Web', this.href)"><?=$r[smt12];?></a></td></tr>
<tr><td>Scan Surat Rekomendasi</td> 	<td> :<a href="../peserta/rekomendasi/<?=$r[rekomendasi];?>" onClick="return GB_showCenter('Tentang Web', this.href)"><?=$r[rekomendasi];?></a></td></tr>
					</table>
				</td>
 	 		</tr>
  			<tr>
    			<tr><td colspan=2 align=center><input type=button value=Kembali onclick=self.history.back() style="cursor:pointer">
						<a href="?module=mhspmdk&act=update&id=<?=$r[i_registrasi]?>" style="text-decoration:none"><input type="button" value="Edit" style="cursor:pointer"></a></td></tr>
...
?>
	<h2>Edit Data Pendaftaran PMDK</h2>
	<? echo "
		  <form method=POST enctype='multipart/form-data' action='$aksi?module=mhspmdk&act=editreg' name='demo'>";?>
	<!--- <form method=POST enctype='multipart/form-data' action="$aksi?module=pmdk&act=editreg" name="demo"> -->
		<table>
 			<tr>
    			<td>
					<table>
...
echo"
    <h2>Input Kelulusan PMDK</h2>
	<form method=POST enctype='multipart/form-data' action='$aksi?module=mhspmdk&act=inputmhsexcel'>
...
echo"
	...
	<form method='POST' enctype='multipart/form-data' action='$aksi?module=mhspmdk&act=pindah'>
