...
$q = mysql_query("SELECT t_kab.nama_kab, t_prop.nama_prop, t_calon_mahasiswa.* FROM t_calon_mahasiswa INNER JOIN t_kab ON t_kab.kd_kab = t_calon_mahasiswa.n_kabupaten INNER JOIN t_prop ON t_prop.kd_prop = t_calon_mahasiswa.n_propinsi WHERE c_jalur='pmdk' ORDER BY i_registrasi ASC");
//$qj = mysql_query("SELECT a.NamaJrsn FROM t_jurusan a WHERE a.Id = $jur");
//$rj = mysql_fetch_array($qj);
?>
<html>
<head>
	<title>Daftar Calon Mahasiswa PMDK</title>
</head>
<body>
<center>
...
<td width="626">
  <div align="center"><b>Daftar Calon Mahasiswa PMDK Politeknik Pos Indonesia<br>
    Tahun Ajaran 
    <?=date('Y')?>
    </b><br>
    <br>
