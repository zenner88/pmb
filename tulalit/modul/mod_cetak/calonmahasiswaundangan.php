...
$q = mysql_query("SELECT t_calon_mahasiswa.*, t_daftar.*
						FROM t_calon_mahasiswa
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi
						
						WHERE t_calon_mahasiswa.c_jalur = 'UNDANGAN'ORDER BY t_calon_mahasiswa.n_sma,t_calon_mahasiswa.n_lengkap ASC");
}
...
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi
						...
						WHERE t_calon_mahasiswa.c_jalur = 'UNDANGAN' and t_daftar.pilihan='1' ORDER BY t_calon_mahasiswa.n_sma,t_calon_mahasiswa.n_lengkap ASC");
}
...
inner join t_daftar on t_daftar.kode_briva=t_calon_mahasiswa.i_registrasi
						...
						WHERE t_calon_mahasiswa.c_jalur = 'UNDANGAN' and t_daftar.pilihan='2' ORDER BY t_calon_mahasiswa.n_sma,t_calon_mahasiswa.n_lengkap ASC");
}
...
if ($pt=='all')
{
echo"<td><h3 align='center'>LAPORAN DATA CALON MAHASISWA UNDANGAN POLTEKPOS DAN STIMLOG <BR />TAHUN ".date('Y')."</h3></td>";
}
elseif ($pt=='poltekpos' or $pt=='stimlog')
{
echo"<td><h3 align='center'>LAPORAN DATA CALON MAHASISWA UNDANGAN ".strtoupper($pt)." <BR />TAHUN".date('Y')."</h3></td>";
}
?>
	...
	while ($r = mysql_fetch_array($q))
	{
		...
		if ($r['jalur_pendaftaran'] == '0') $jalur = 'UNDANGAN';
		else $jalur = 'REGULER';
